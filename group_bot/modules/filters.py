...
async def imp(client, m):
    ...
    if not m.reply_to_message:
        return
    status = str((await client.get_chat_member(chat_id=chat_id, user_id=user_id)).status)
            ...
            if os.path.isfile(f"db/X{chat_id}_db.txt"):
                ...
                try:
                    ...
                    with open(f"db/X{chat_id}_db.txt", 'w+') as wf:
                        ...
                        await m.reply_text(
                            f"Filter {a_u} for '`{html.escape(trigger)}`' with action on '**{action_on}**' !")
                        ...
                        if i['trigger'] == trigger:
                            ...
                            if not len(str(obj)) < 20:
                                with open(f"db/X{chat_id}_db.txt", 'w+') as wf:
                                    json.dump(obj, wf, indent=4)
                                    await m.reply_text(
                                        f"Filter deleted for '`{html.escape(trigger)}`' !")
                                    break
            else:
                await m.reply_text(
...
async def getnotes(client, m):
    ...
    if is_enabled is not True:
        return
    if not " " in m.text:
         await m.reply_text("Mention note name along with command !")
         return 
    if m.from_user:
        uid = m.from_user.id
    else:
...
async def noteslist_callback(client, m, chat_id):
    ...
    if len(str(triggers)) > 3:
        for i in triggers:
            ...
            filterslist.append(
                f"- [{html.escape(word)}](t.me/{bot_username}?start={chat_id}_NOTES_{notecode})")
        filterslist.sort()       
        filterslists = "\n".join(filterslist)
...
async def verify_captcha(client, m, chat_id):
    user_id = m.from_user.id
    verifying = await m.reply_text("`verifying ...`\nIf issue arrives contact @Ashit_10", quote=True)
    ...
    elif (status == "ChatMemberStatus.MEMBER") or (stat.permissions.can_send_messages) or (int(user_id) in oldNew):
...
async def antipin(client, m):
    if m.sender_chat and m.views:
        ...
        if int(m.sender_chat.id) == int(my_ch):
            try:
                await client.unpin_chat_message(chat_id=chat_id, id=m.id)
            except:
                pass
